...
class MyResNet_40(ResNet):
    def __init__(self):
        super(MyResNet_40,self).__init__(BasicBlock, [3, 4, 6, 3], 1000)
        self.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
...
class DepthResNet(nn.Module):
    def __init__(self):
        ...
        nn.init.constant_(self.conv1.weight, 1.0 / 49.0)
        # nn.init.constant(self.layer1.weight, 1.0 / 49.0)
        nn.init.constant_(self.layer2.weight, 1.0)
...
class RelevanceMap(nn.Module):
    ...
    def __init__(self, wordEmbeddingNum,wordEmbeddingDim, featureChannel=256, mapChannel=256):
    ...
    def forward(self, featureTopView, word):
        ...
        for i in range(0, batchSize):
            _feature = featureTopView[i].contiguous().view(1, self.featureChannel, H, W)
