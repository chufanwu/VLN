...
from goalmap_utils import buildVocaulary, buildEmbeddingVocabulary, Tokenizer, sort_batch,generateTopView
from goalmap_dataset import DatasetGenerator
...
class Task():
    def __init__(self, args):
        ...
        # Path
        # self.jsonDataPath = '/home/qiyuand/matterport3D/goalmap/training_data.json'
        self.jsonDataPath = '/home/qiyuand/GoalMap/AnnotationDataGoalMap/training_data_10.json'
        ...
        self.trainDataset = DatasetGenerator(jsonDataPath=self.jsonDataPath, ImageRootPath=self.imageRootPath)
        ...
        mapParams = list(self.map.module.linearLayer3.parameters())
        mapParams += list(self.map.module.goalMapBN.parameters())

        self.mapOptimizer = optim.Adam(mapParams, lr=self.lrRele, betas=(0.9, 0.999),
                                       eps=1e-08,
                                       weight_decay=self.weightDecay)
    ...
    def epochTrain(self):
        ...
        for idx, (rgbImage, depthImage, goalTarget, instruction) in enumerate(self.trainDataLoader):
            # rgbImage: batch * 3 * 224 * 224, tensor
            # depthImage: batch * 1 * 28 * 28, numpy
            ...
            rgbImage = rgbImage[perm_idx]
            depthImage = depthImage[perm_idx.detach().cpu().numpy()]
            goalTarget = goalTarget[perm_idx]
            instruction_idx = instruction_idx[perm_idx]


            # Go through the models
            feature = self.rgbResNetModel(rgbImage) # 1024 * 28 * 28
            releMap,releMapTopView, goalMap = self.map(feature, depthImage, instruction_idx,seq_lengths,False) # depth in numpy
            lossValue = self.loss(input=goalMap, target=goalTarget)


            # # Visualization
            try:
                if (idx + 1) % self.batchPerVisualize == 0 and self.visualize:
                    rgb_v = rgbImage[0, :, :, :].detach().cpu().numpy() # 3 * 448 * 448, numpy
                    depth_v = depthImage[0,:,:] # 28 * 28, numpy
                    goalmapTarget_v = goalTarget[0, :, :].detach().cpu().numpy() # 28 * 28, numpy
                    relemap_v = releMap[0,:,:].detach().cpu().numpy() # 1 * 28 * 28, numpy
                    relemapTopView_v = releMapTopView[0,:,:].detach().cpu().numpy()
                    goalmap_v = goalMap[0,:,:].detach().cpu().numpy() # 28 * 28, numpy
                    instruction_v = instruction_idx[0].detach().cpu().numpy()
                    instruction_v = self.tokenizer.decode_sentence(instruction_v) # string

                    self.visualization(rgb_v,depth_v,relemap_v,relemapTopView_v, goalmap_v, goalmapTarget_v, instruction_v , idx + 1)
            except Exception, e:
                print e
            ...
            del feature, releMap,goalMap, rgbImage, depthImage, goalTarget, instruction_idx
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
    ...
    def load(self, path):
        ...
        # self.relevanceOptimizer.load_state_dict(modelCheckpoint['map_optimizer'])
    ...
    def visualization(self, rgb_v,depth_v,relemap_v,relemapTopView_v, goalmap_v, goalmapTarget_v, instruction_v , batchIdx, test=False):

        plt.switch_backend('agg')
        plt.figure(figsize=(35, 35), dpi=300)

        plt.figure()
        plt.suptitle(instruction_v, fontsize=16)
        ...
        plt.subplot(2, 3, 2)
        plt.imshow(relemap_v)

        plt.subplot(2, 3, 4)
        plt.imshow(relemapTopView_v)

        plt.subplot(2, 3, 5)
        plt.imshow(goalmap_v)

        plt.subplot(2, 3, 6)
        plt.imshow(goalmapTarget_v)
